;(function() {
	...
	function Hero() {
		...
		this.element  = document.createElement('div');
		this.element.style.background = 'url(images/freiza.png)';
		...
		var animateSprite = function() {
			var currentX = 80 * currentSpriteNoX;
			that.element.style.backgroundPositionX = currentX +'px';
			if(currentSpriteNoX == 5) {
				currentSpriteNoX = 0;
			}
			else {
				currentSpriteNoX += 1;
		...
		this.animation = setInterval(animateSprite,125);
	...
	function LineRunner(gameDiv_) {
		...
		gameTitle.innerHTML = "FRIEZA";
		gameTitle.style.position='absolute';
		gameTitle.style.left='320px';
		gameTitle.style.font='bold 45px arial';
